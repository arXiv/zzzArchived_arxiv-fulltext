...
from .. import controllers, domain
...
class TestTaskStatus(TestCase):
    ...
    @mock.patch(f'{controllers.__name__}.get_extraction_task_status')
    def test_task_is_pending(self, mock_get_status):
    ...
    @mock.patch(f'{controllers.__name__}.get_extraction_task_status')
    def test_task_is_sent(self, mock_get_status):
        """We set to SENT on creation."""
        mock_get_status.return_value = 'SENT'
        data, code, headers = controllers.get_task_status('task12345')
        self.assertEqual(code, status.HTTP_200_OK)

    @mock.patch(f'{controllers.__name__}.get_extraction_task_status')
    def test_task_is_started(self, mock_get_status):
        """Task is started."""
    ...
    @mock.patch(f'{controllers.__name__}.get_extraction_task_status')
    def test_task_is_retrying(self, mock_get_status):
        """Task is retrying."""
        mock_get_status.return_value = 'RETRY'
        data, code, headers = controllers.get_task_status('task12345')
        self.assertEqual(code, status.HTTP_200_OK)

    @mock.patch(f'{controllers.__name__}.get_extraction_task_result',
                mock.MagicMock(return_value='No good!'))
    @mock.patch(f'{controllers.__name__}.get_extraction_task_status')
    def test_task_failed(self, mock_get_status):
        """Task failed for good."""
        mock_get_status.return_value = 'FAILURE'
        data, code, headers = controllers.get_task_status('task12345')
        self.assertEqual(code, status.HTTP_200_OK)
    ...
    @mock.patch(f'{controllers.__name__}.get_extraction_task_result')
    @mock.patch(f'{controllers.__name__}.get_extraction_task_status')
    def test_task_succeeded(self, mock_get_status, mock_get_result,
                            mock_url_for):
        """Task succeeded."""
    ...
    @mock.patch(f'{controllers.__name__}.get_extraction_task_result')
    @mock.patch(f'{controllers.__name__}.get_extraction_task_status')
    def test_sub_task_succeeded(self, mock_get_status, mock_get_result,
                                mock_url_for):
        """Task succeeded."""
        mock_get_status.return_value = 'SUCCESS'
        mock_get_result.return_value = {
    ...
    @mock.patch(f'{controllers.__name__}.get_extraction_task_result')
    @mock.patch(f'{controllers.__name__}.get_extraction_task_status')
    def test_task_succeeded_oddly(self, mock_get_status, mock_get_result):
